...
import { userLogin, getUserInfo } from "@/api/manager"
...
async function handleLogin() {
  ...
  try {
    const res = await userLogin({ username: form.username.trim(), password: encryptPassword(form.password) })
    isLoading.value = false
    if (res.data.code === 200) {
      store.setToken(res.data.data)
      try {
        const infoRes = await getUserInfo()
        if (infoRes.data.code === 200) store.setUserInfo(infoRes.data.data)
      } catch {}
      store.fetchCartList()
      ...
      <div class="auth-form-panel">
        <div class="auth-form-inner">
          <div class="auth-form-header">
            <h1>用户登录</h1>
            <p>还没有账户？<router-link to="/user/register" class="auth-link">立即注册</router-link></p>
          </div>
          ...
          <el-form ref="formRef" :model="form" :rules="rules" label-position="top" @keyup.enter="handleLogin">
            ...
            <div class="form-options">
              <label class="remember-me">
                <input type="checkbox" class="custom-checkbox" v-model="rememberMe" />
                <span class="checkbox-label">记住我</span>
              </label>
              <a class="forgot-link" @click="handleForgotPassword">忘记密码？</a>
            </div>

            <el-button type="primary" @click="handleLogin" class="auth-submit" :loading="isLoading">
...
.forgot-link {
  ...
  &::after {
    ...
    width: 0;
    height: 2px;
    background: $accent-vitality;
    transition: width 0.3s ease;
    border-radius: 1px;
  }

  &:hover {
    color: darken($accent-vitality, 10%);
